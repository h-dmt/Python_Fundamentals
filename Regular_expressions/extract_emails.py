"""
a program that receives a single string and extracts all email addresses from it.
Print the extracted email addresses on separate lines. Emails are in the format
"{user}@{host}", where:
    • {user} could consist only of letters and digits; the symbols ".", "-" and
    ...
    • {host} is a sequence of at least two words, each couple of words must
        be separated by a single dot ".". Each word consists of only letters and
        can have hyphens "-" between the letters.
        ◦ Examples of valid hosts: "softuni.bg", "software-university.com",
...
some_text = input()
emails = re.findall(regex, some_text)
for email in emails:
    print(email[0])
