...
representing targets and their value. Afterward, you will be receiving indices
until the "End" command is given, and you need to print the targets and the count
of shot targets.
...
Along with that, you also need to:
    • Reduce all the other targets, which have greater values than your current
    target, with its value.
    • Increase all the other targets, which have less than or equal value to the
    shot target, with its value.
...
while index != 'End':
    ...
    if index < 0 or index >= len(targets) \
        ...
        for i in range(len(targets)):
            if targets[i] > cur_target:
                if targets[i] != -1:
                    targets[i] -= cur_target
            elif targets[i] <= cur_target:
                if targets[i] != -1:
                    targets[i] += cur_target
        targets[index] = -1
        # print(targets)
...
print(f"Shot targets: {target_shot} -> ", end='')
print(' '.join(targets))
